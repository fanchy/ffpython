...
struct pyops_t
{
    static int traceback(std::string& ret_);
};
struct cpp_void_t{};
...
struct pycall_t
{
    static int call_func(PyObject *pModule, const std::string& mod_name_, const std::string& func_name_,
                         pycall_arg_t& pyarg_, pytype_tool_t& pyret_, std::string& err_)
    {
        ...
        if (pFunc && PyCallable_Check(pFunc)) {
            ...
            if (pValue != NULL) {
                if (pyret_.parse_value(pValue))
                {
                    ...
                    err_ += std::string(" ") + func_name_  + " in " + mod_name_;
                }
    ...
    static int call_func_obj(PyObject *pFunc, pycall_arg_t& pyarg_, pytype_tool_t& pyret_, std::string& err_)
    {
        if (pFunc && PyCallable_Check(pFunc)) {
            PyObject *pArgs = pyarg_.get_args();
            PyObject *pValue = PyObject_CallObject(pFunc, pArgs);
    ...
    template<typename T>
    static const T& call(const std::string& mod_name_, const std::string& func_name_, pycall_arg_t& pyarg_, pytype_tool_impl_t<T>& pyret);
    template<typename T>
    static const T& call_obj_method(PyObject *pObj, const std::string& func_name_, pycall_arg_t& pyarg_, pytype_tool_impl_t<T>& pyret);
    template<typename T>
...
struct cpp_to_pyclass_reg_info_t
{
    struct inherit_info_t
    {
        ...
        std::set<PyTypeObject*> all_child;
    };
    typedef std::map<std::string, inherit_info_t> inherit_info_map_t;
    ...
    static void add(const std::string& child_, const std::string& base_, PyTypeObject* def_)
    {
        inherit_info_t tmp;
    ...
    static bool is_instance(PyObject* pysrc, const std::string& class_)
    {
        ...
        for (std::set<PyTypeObject*>::iterator it = tmp.all_child.begin(); it != tmp.all_child.end(); ++it)
        {
...
public:
    ...
    std::string                class_name;
    std::string                class_real_name;
    std::string                class_name_with_mod;
    std::string                class_reel_name_with_mod;
    std::string                inherit_name;
    int         type_size;
    std::string      doc;
    int         args_num;
    ...
    std::vector<method_info_t>        methods_info;
    //! property 
    std::vector<property_info_t>        propertys_info;
    //! for init module
    PyTypeObject                        pytype_def;
    //! method
    std::vector<PyMethodDef>                pymethod_def;
    //! property
    std::vector<PyGetSetDef>     pyproperty_def;

    //! 静态类型需要全局记录该类型被注册成神马python 类型
    static_pytype_info_t*   static_pytype_info;

    template<typename FUNC>
    pyclass_regigster_tool_t& reg(FUNC f_, const std::string& func_name_, std::string doc_ = "")
    {
        method_info_t tmp;
    ...
    template<typename CLASS_TYPE,typename RET>
    pyclass_regigster_tool_t& reg_property(RET CLASS_TYPE::* member_, const std::string& member_name_, std::string doc_ = "")
    {
        property_info_t tmp;
...
struct pytype_traits_t<const T*>
{
    ...
    {
        const std::string& mod_name = pyclass_base_info_t<T>::pytype_info.mod_name;
        const std::string& class_name = pyclass_base_info_t<T>::pytype_info.class_name;
        PyObject *pName = NULL, *pModule = NULL, *pValue = NULL;

        if (class_name.empty())
            return pytype_traits_t<PTR_NUMBER>::pyobj_from_cppobj(PTR_NUMBER(val_));
...
struct pytype_traits_t<std::string>
{
    ...
    static int pyobj_to_cppobj(PyObject *pvalue_, std::string& m_ret)
    {
        if (true == PyString_Check(pvalue_))
        {
...
struct pytype_traits_t<std::vector<T> >
{
    ...
    static int pyobj_to_cppobj(PyObject *pvalue_, std::vector<T>& m_ret)
    {
        m_ret.clear();
...
struct pytype_traits_t<std::list<T> >
{
    static PyObject* pyobj_from_cppobj(const std::list<T>& val_)
    {
        ...
        int i = 0;
        for (typename std::list<T>::const_iterator it = val_.begin(); it != val_.end(); ++it)
        {
            PyList_SetItem(ret, i++, pytype_traits_t<T>::pyobj_from_cppobj(*it));
        }
        return ret;
    }
    static int pyobj_to_cppobj(PyObject *pvalue_, std::list<T>& m_ret)
    {
        pytype_tool_impl_t<T> ret_tool;
        if (true == PyTuple_Check(pvalue_))
        {
...
struct pytype_traits_t<std::set<T> >
{
    static PyObject* pyobj_from_cppobj(const std::set<T>& val_)
    {
        ...
        for (typename std::set<T>::const_iterator it = val_.begin(); it != val_.end(); ++it)
        {
    ...
    static int pyobj_to_cppobj(PyObject *pvalue_, std::set<T>& m_ret)
    {
        m_ret.clear();
        pytype_tool_impl_t<T> ret_tool;
...
struct pytype_traits_t<std::map<T, R> >
{
    static PyObject* pyobj_from_cppobj(const std::map<T, R>& val_)
    {
        ...
        for (typename std::map<T, R>::const_iterator it = val_.begin(); it != val_.end(); ++it)
        {
    ...
    static int pyobj_to_cppobj(PyObject *pvalue_, std::map<T, R>& m_ret)
    {
        m_ret.clear();
